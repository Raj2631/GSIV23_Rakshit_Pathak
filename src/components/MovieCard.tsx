...
const MovieCard = ({ movie }: Props) => {
  ...
  return (
    <div className="w-44 rounded-lg cursor-pointer overflow-hidden shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,_rgba(0,0,0,0.3)_0px_3px_7px_-3px]">
      <Link to={`/movieDetail/${movie.id}`}>
        <img
          key={movie.id}
          src={`${ImgBaseURL}/${movie.poster_path}`}
          className="h-64 w-44 bg-black bg-cover"
        />
